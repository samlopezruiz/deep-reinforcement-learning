...
class ReplayBuffer:
    ...
    def sample(self):
        ...
        weights = torch.from_numpy(np.ones(len(rewards))).unsqueeze(1).to(device)
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed, epsilon=1e-5, alpha=0.6, beta=0.4):
        ...
        ======
            ...
            alpha (float): exponent for probabilities
            beta (float): exponent for importance sampling weights
        """
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        e = self.experience(state, action, reward, next_state, done)
        self.memory.append(e)
        self.priorities.append(max(self.priorities, default=self.epsilon))
    ...
    def update_priorities(self, indices, errors):
        ...
        for idx, error in zip(indices, errors):
            self.priorities[idx] = np.abs(error.detach().cpu().numpy()[0]) + self.epsilon
